(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["xmpEditor"] = factory();
	else
		root["xmpEditor"] = factory();
...
var xmp_api_helpers_namespaceObject = {};
__webpack_require__.r(xmp_api_helpers_namespaceObject);
__webpack_require__.d(xmp_api_helpers_namespaceObject, "dataUrlToArrayBuffer", function() { return dataUrlToArrayBuffer; });
__webpack_require__.d(xmp_api_helpers_namespaceObject, "urlToArrayBuffer", function() { return urlToArrayBuffer; });
__webpack_require__.d(xmp_api_helpers_namespaceObject, "createNewXmp", function() { return createNewXmp; });
__webpack_require__.d(xmp_api_helpers_namespaceObject, "blobToArrayBuffer", function() { return blobToArrayBuffer; });
__webpack_require__.d(xmp_api_helpers_namespaceObject, "formatXml", function() { return formatXml; });
__webpack_require__.d(xmp_api_helpers_namespaceObject, "arrayBufferToDataUrl", function() { return arrayBufferToDataUrl; });
...
var base64_js = __webpack_require__("1fb5");
var base64_js_default = /*#__PURE__*/__webpack_require__.n(base64_js);

// CONCATENATED MODULE: ./src/js/xmp-api-helpers.js
...
function formatXml(xml) {
  var PADDING = ' '.repeat(2); // set desired indent size here
...
function imgToArrayBuffer(obj) {
  return Promise.resolve().then(function () {
    if (obj.src) {
      // Data URI
      if (/^data\:/i.test(img.src)) return Promise.resolve(base64ToArrayBuffer(img.src)); // Object URL
      else if (/^blob\:/i.test(img.src)) return blobUrlToArrayBuffer(img.src); // URL
        else return urlToArrayBuffer(img.src);
    } else {
      var isBlob = img instanceof window.Blob || img instanceof window.File;
...
function urlToArrayBuffer(url) {
  return new Promise(function (resolve, reject) {
    ...
    http.onload = function () {
      if (this.status == 200 || this.status === 0) resolve(http.response);else reject('Could not load image from URL: ' + url);
      http = null;
    };
...
function _blobToArrayBuffer() {
  _blobToArrayBuffer = _asyncToGenerator(
  /*#__PURE__*/
...
function blobUrlToArrayBuffer(blobUrl) {
  return new Promise(function (resolve, reject) {
    ...
    var http = new XMLHttpRequest(); // TODO: use 'arrayBuffer' instead of blob (might not need blobToArrayBuffer conversion)
...
function dataUrlToArrayBuffer(dataUrl) {
  ...
  var arrBuf = base64_js_default.a.toByteArray(base64);
  return arrBuf;
}

function arrayBufferToDataUrl(mimeType, arrBuf) {
  var base64 = base64_js_default.a.fromByteArray(new Uint8Array(arrBuf));
  var dataUrl = "data:".concat(mimeType, ";base64,").concat(base64);
  return dataUrl;
...
//   return json;
// }
// CONCATENATED MODULE: ./src/js/xmp-api.js

// TODO: add XMP size limit warning
// TODO: compress XMP white space

var XMP_HEADER = 'http://ns.adobe.com/xap/1.0/\x00';
var XMP_END_META = '</x:xmpmeta>';
var XMP_END_PACKETS = ["<?xpacket end=\"w\"?>", "<?xpacket end='w'?>"];
var JPG_SEGMENT_MARKER_LENGTH = 4;
var JPG_FILE_MARKER = [0xFF, 0xD8];
var JPG_APP1_MARKER = [0xFF, 0xE1];
var JPG_MARKERS = {
  0xD8: 'Start Of Image',
...
function jpgReadXmp(buffer) {
  var _jpgFindXmpSegment = jpgFindXmpSegment(buffer),
      markerOffset = _jpgFindXmpSegment.markerOffset,
      segmentLength = _jpgFindXmpSegment.segmentLength;
...
function jpgWriteXmp(buffer, xmpString) {
  ...
  var _jpgFindXmpSegment2 = jpgFindXmpSegment(buffer),
      markerOffset = _jpgFindXmpSegment2.markerOffset,
      segmentLength = _jpgFindXmpSegment2.segmentLength; // insert at existing XMP segment offset or right after file start marker


  markerOffset = markerOffset || JPG_FILE_MARKER.length;
  var oldSegmentLength = segmentLength || 0;
  var newSegmentLength = jpgGetXmpSegmentLength(xmpString, false, false);
  var odlArr = new Uint8Array(buffer);
  var newArr = new Uint8Array(odlArr.length - oldSegmentLength + newSegmentLength); // write file part before XMP segment

  copyArr(newArr, 0, odlArr, 0, markerOffset); // insert XMP segment

  jpgWriteXmpSegment(newArr, markerOffset, xmpString); // file part after XMP segment 
...
function jpgRemoveXmp(buffer) {
  ...
  var _jpgFindXmpSegment3 = jpgFindXmpSegment(buffer),
      markerOffset = _jpgFindXmpSegment3.markerOffset,
      segmentLength = _jpgFindXmpSegment3.segmentLength; // file contains no XMP: nothing to remove


  if (!markerOffset || !segmentLength) return buffer;
  var odlArr = new Uint8Array(buffer);
  var newArr = new Uint8Array(odlArr.length - segmentLength); // write file part before XMP segment

  copyArr(newArr, 0, odlArr, 0, markerOffset); // file part after XMP segment 
...
function jpgGetXmpSegmentLength(xmpString, truncate, withoutMarker) {
  var length = JPG_SEGMENT_MARKER_LENGTH + XMP_HEADER.length;
...
function jpgFindXmpSegment(buffer) {
  var arr = new Uint8Array(buffer);
  var dv = new DataView(buffer); // const arr16 = new Uint16Array(buffer)

  var markerOffset;
  var segmentLength;
  var segmentLengthFromHeader;
  var offset = 0;
  var segments = {};
  var first219Marker;
  var firstQtMarker; // while (!segmentLength && offset < arr.length) {
  ...
  while (offset < arr.length) {
    ...
    if (!markerOffset) {
      if (arr[offset] === 0xFF) {
        ...
        if (arr[offset + 1] === 0xE1 && getStrFromUint8(arr, offset + 4, XMP_HEADER.length) === XMP_HEADER) {
          markerOffset = offset;
          segmentLengthFromHeader = dv.getUint16(offset + 2);
        }
      }
    } else if ( // 2. Find XMP segment end
    !segmentLength && arr[offset] === 0x3C && arr[offset + 1] === 0x3F // ascii char: '<?'
    ) {
        XMP_END_PACKETS.forEach(function (p) {
  ...
  return {
    ...
    segmentLengthFromHeader: segmentLengthFromHeader,
    first219Marker: first219Marker,
    firstQtMarker: firstQtMarker
  };
}
...
function copyArr(targetArr, targetPos, sourceArr, sourcePos, sourceLen) {
  ...
  for (var i = 0; i < sourceLen; i++) {
    targetArr[targetPos + i] = sourceArr[sourcePos + i];
  }
}
...
/* concated harmony reexport jpgReadXmp */__webpack_require__.d(__webpack_exports__, "jpgReadXmp", function() { return jpgReadXmp; });
/* concated harmony reexport jpgWriteXmp */__webpack_require__.d(__webpack_exports__, "jpgWriteXmp", function() { return jpgWriteXmp; });
/* concated harmony reexport jpgRemoveXmp */__webpack_require__.d(__webpack_exports__, "jpgRemoveXmp", function() { return jpgRemoveXmp; });
/* concated harmony reexport helpers */__webpack_require__.d(__webpack_exports__, "helpers", function() { return xmp_api_helpers_namespaceObject; });
...
//# sourceMappingURL=xmpEditor.umd.js.map